...
```powershell
$env:TF_VAR_name_suffix = $env:SUFFIX
terraform init
```
...
Both changes are reasonable. Neither is in code. **This is the most common way a
repository stops matching what is actually deployed.**
...
Format from a terminal, then use the **Source Control** panel:
...
1. Stage `environments/dev/main.tf` with the **+**.
2. Commit message:

   ```
   Accept INC-4471 tag from portal change, drop temporary 8080 rule
   ```

3. Click the **Commit** checkmark, then **Publish Branch**.
4. Click the **GitHub** icon, then **Create Pull Request**.
...
Get it reviewed, merge, and watch the apply. Then switch to `main` with the
status bar branch picker, click the sync icon, and check the plan from a
terminal:
...
Keep everything. The final lab handles teardown.
