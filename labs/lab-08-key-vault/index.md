...
variable you set by hand, your shell history, a sticky note, or, one bad
afternoon, a `.tfvars` file that gets committed.
...
the pipeline has to make the introduction. You will do that grant by hand in Lab
10, and it is worth noticing that this one manual step is the root of trust for
...
with a short-lived token and there is no stored secret at all. It is where you
should go next after this course. Lab 10 uses a service principal because that
