...
Dev stays hand-written for now. Converting it needs a technique you do not have
yet.
...
still creates the resource group directly: a resource group is a single resource,
which is not worth wrapping in a module.
...
where dev gets `Standard_F1als_v7`. That is what the variable and directory work
bought you: two environments with different sizes and address spaces, no
duplicated resource definitions, and no shared state.
...
Look at what changed. You did not change a single argument in your
configuration. You changed four characters in a URL, and your production storage
account's security posture changed. That is why you pin a module version:
...
shell, somebody's history, and definitely in state. That is the next thing to
fix.
