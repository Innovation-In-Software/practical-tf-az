...
Summit's two environments are built differently. `environments/prod` is sixty
lines of module calls. `environments/dev` is a hundred and thirty lines of
hand-written resources, because you wrote it before you knew modules existed.

That has practical consequences. Because they are built differently, a
...
4. Click the branch name again, choose **Create new branch...**, and name it:
   ...
   ```
   feature/lab12-dev-on-modules
   ```

5. Confirm the status bar now shows `feature/lab12-dev-on-modules`.
...
### Set your variable and check the plan
...
```
Error: A resource with the ID "/subscriptions/.../storageAccounts/stsummitordersdevjr63"
...
Terraform will perform the following actions:
  ...
  # azurerm_storage_account.orders has moved to module.storage.azurerm_storage_account.this
    resource "azurerm_storage_account" "this" {
        id = "/subscriptions/.../stsummitordersdevjr63"
        name = "stsummitordersdevjr63"
    }
```
...
**The pipeline is the deciding factor.** Nobody runs Terraform by hand any more.
A refactor that requires somebody to type a state command on their laptop cannot
be delivered by your pipeline at all. `moved` is a line of code, so it
...
# ---------------------------------------------------------------------------
# The resource group stays inline. A single resource is not worth a module.
# ---------------------------------------------------------------------------
...
**Replace the whole contents of `environments/dev/moved.tf`.** You already have
that file from Lab 6, holding the single block that renamed the storage container.
That move was applied long ago, so the block has done its job and can go. A
separate file keeps the refactor visible in the pull request and makes it easy to
delete later.
...
Then in the **Source Control** panel: stage everything with the **+**, and use
this as the commit message. VS Code accepts multiple lines, so put the summary on
the first line and the detail below it.

```
Refactor dev onto shared modules using moved blocks
...
Click the **Commit** checkmark, then **Publish Branch**.
...
**A reviewer can confirm nothing is destroyed without running anything.**
...
Switch back to `main` with the status bar branch picker and click the sync icon.
...
```powershell
cd C:\Users\Administrator\Downloads\terraform\labs\az-tf-ops-<your-username>
.\scripts\destroy-all.ps1 -Suffix $env:SUFFIX -WhatIf
```

Then run it:

```powershell
.\scripts\destroy-all.ps1 -Suffix $env:SUFFIX
```
...
az keyvault delete --name "kv-summit-dev-$env:SUFFIX"  --resource-group rg-summit-security
az keyvault delete --name "kv-summit-prod-$env:SUFFIX" --resource-group rg-summit-security
az keyvault purge  --name "kv-summit-dev-$env:SUFFIX"  --location eastus
az keyvault purge  --name "kv-summit-prod-$env:SUFFIX" --location eastus
...
| A script under `scripts/` fails in a way the lab does not describe | Your copy predates a fix to the template. Run `git fetch https://github.com/Innovation-In-Software/az-tf-ops.git main` then `git checkout FETCH_HEAD -- scripts/`. |
| `running scripts is disabled on this system` | Windows blocks script files by default. Run `Set-ExecutionPolicy -Scope CurrentUser -ExecutionPolicy RemoteSigned -Force`, then rerun the script. |
...
You restructured a live environment from top to bottom, under a running server,
with nothing destroyed, and shipped it through a pipeline where a reviewer could
verify that before it merged.

Then you destroyed the environment and rebuilt it from a file, which is the
claim infrastructure as code makes and the one worth being able to demonstrate.
