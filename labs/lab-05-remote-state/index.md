...
the modules they call, which arrive in Lab 7.
...
> the answer is `terraform init -backend-config=...`, which you will see in Lab
> 10 when the pipeline supplies it.
...
Production does not exist yet. It gets its resources in Lab 7, built from shared
modules. For now, give it a directory, a backend, and just enough to prove the
separation is real.
...
has public access disabled and why access to it is a privilege. Lab 8 addresses
...
>
> Lab 12 introduces the `moved` block, which does the same job as a line of code
> that travels through the pull request and runs automatically. Use `moved` for
> renames you are committing. Keep `state mv` for interactive repair work.
...
The two directories are near-duplicates right now. Lab 6 makes that stop.
